    ...
    students.slice(0, 500).forEach(s => {
        tbody.innerHTML += `
            <tr>
                ...
                <td class="total-col" style="font-weight:bold;color:var(--primary);">${getAvgGrade(s)}</td>
            </tr>`;
    });
            ...
            <div class="grade-filter-btns">
                ${Array.from({length: 9}, (_, i) => i + 1).map(g => `
                    <button class="grade-filter-btn g-btn-${sub.k}-${g}"
                        onclick="renderGradeFilter('${sub.k}', ${g})"
...
window.renderGradeFilter = function(subjectKey, grade) {
    // 이전 활성 버튼 해제 후 현재 버튼 활성화
    document.querySelectorAll('.grade-filter-btn.active').forEach(b => b.classList.remove('active'));
    const activeBtn = document.querySelector(`.g-btn-${subjectKey}-${grade}`);
    ...
    const filtered = students
        .filter(s => s[subjectKey].grd === grade)
        .sort((a, b) => {
            const diff = b[subjectKey].raw - a[subjectKey].raw;
    ...
    const subjectNames = { kor: '국어', math: '수학', eng: '영어', hist: '한국사', inq1: '사회탐구', inq2: '과학탐구' };
    const subjectName  = subjectNames[subjectKey];
    const isAbs        = (subjectKey === 'eng' || subjectKey === 'hist');

    summaryEl.innerHTML = `
        <span class="grade-filter-summary-text">
            <strong>${subjectName}</strong>
            <span class="grade-badge g-${grade}">${grade}등급</span>
            해당 학생 <strong>${filtered.length}명</strong>
        </span>`;

    thead.innerHTML = `<tr>
        <th>학번</th><th>이름</th><th>원점수</th><th>등급</th>
        ${!isAbs ? '<th>표준점수</th><th>백분위</th>' : ''}
    </tr>`;

    tbody.innerHTML = filtered.map(s => {
        const sub = s[subjectKey];
...
function renderClass() {
    ...
    if (tbody) {
        ...
        students.forEach(s => {
            ...
            html += `<td class="total-col">${getTot(s)}</td><td class="total-col">${rank}</td><td class="total-col" style="font-weight:bold;color:var(--primary);">${getAvgGrade(s)}</td></tr>`;
    ...
    if (container) {
        ...
        subjects.forEach(sub => {
            ...
            if ((k === 'inq1' || k === 'inq2') && currentData[k].name && currentData[k].name !== '-') {
                // sub.n이 이미 '사회탐구'/'과학탐구'이므로 과목명만 표시
                displayTitle = sub.n;
            }
...
async function captureTabPageV2(pdf, showSelectors, addNewPage) {
    ...
    if (showSelectors.includes('charts')) {
        ...
        if (chartsRow) {
            ...
            // chart-container: auto height so image aspect ratio drives the size
    ...
    allSubjects.forEach(subj => {
        ...
        cc.querySelectorAll('.subject-detail-grid').forEach(grid => {
            ...
            // 차트 컨테이너 높이 - auto로 이미지 비율에 맞게 자동 결정
    ...
    wrapper.querySelectorAll('canvas').forEach(cloneCanvas => {
        ...
        // aspect-ratio를 CSS로 명시하여 width:100%로 늘어도 비율 보존
        const ar = snap.w > 0 && snap.h > 0 ? `${snap.w} / ${snap.h}` : 'auto';

        if (isRadar) {
            // 레이더: 컨테이너 폭에 맞추되 원본 비율 유지
            img.style.cssText = [
                'display:block',
                'width:100%',
                'height:auto',
            ...
            // 총점 추이: 컨테이너 폭에 맞추고 원본 비율로 높이 자동 결정
            img.style.cssText = [
                'display:block',
                'width:100%',
                'height:auto',
            ...
            // 과목별 상세 차트: 컨테이너 폭에 맞추고 원본 비율 유지
            img.style.cssText = [
                'display:block',
                'width:100%',
                'height:auto',
